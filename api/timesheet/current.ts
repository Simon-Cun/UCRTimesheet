...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (listResp.status === 302 || (listResp.data as string).toLowerCase().includes('cas/login')) {
      return res.status(401).json({ error: 'SESSION_EXPIRED' });
    }

    // Step 2: Parse the current timesheet link — the one with " - " date range text
    // The Playwright bot clicks page.locator("text= - ").first which is the date range link
    const listHtml = listResp.data as string;
    const info = parseTimesheetListLink(listHtml, appCookie, username);
    if (!info) {
      return res.status(502).json({ error: 'Could not find current timesheet on list page' });
    }
...
function parseTimesheetListLink(
  ...
  const labelMatch = html.match(/<B>([^<]*\d{4}[^<]*-[^<]*\d{4}[^<]*)<\/B>/i)
